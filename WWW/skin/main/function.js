...
function wait(){
	$.blockUI({message:'<h3 id="loading_msg">数据提交中，请稍后....</h3>',overlayCSS:{backgroundColor: '#000',opacity: 0.5,cursor:'wait'}});
}
			...
			if(!data){
				...
				try{
					...
					if(type==73||type==74||type==78){
						$('.pk10_ul_line').removeClass().addClass('k8').css({"width":"200px"});
					}else{
...
function safeBeforSetPwd(){
	...
	if(this.qrpassword.value.length<6){xingcai("新登入密码至少6位");return false;}
...
function safeBeforSetCoinPwd(){
	if(!this.newpassword.value){xingcai("请输入新密码");return false;}
	if(this.newpassword.value.length<6){xingcai("密码至少6位");return false;}
	var confirmpwd=$(':password.confirm', this).val();
	if(confirmpwd!=this.newpassword.value){xingcai("两次输入密码不一样");return false;}
	return true;
}
function safeBeforSetCoinPwd2(){
	...
	if(this.oldpassword.value.length<6){xingcai("旧资金密码至少6位");return false;}
	if(this.oldpassword.value.length>13){xingcai("旧资金密码至多13位");return false;}
	
	if(!this.newpassword.value){$.alert("请输入新资金密码");return false;}
	if(this.newpassword.value.length<6){xingcai("新资金密码至少6位");return false;}
	
	if(!this.qrpassword.value){$.alert("请确认新资金密码");return false;}
	if(this.qrpassword.value.length<6){xingcai("新资金密码至少6位");return false;}
	var confirmpwd=$(':password.confirm', this).val();
	if(confirmpwd!=this.newpassword.value){xingcai("两次输入资金密码不一样");return false;}
	return true;
}
...
function safeBeforSetCBA(){
	...
	if(!this.username.value){xingcai("银行开户姓名没有填写");return false;}
	if(!this.countname.value){xingcai("银行开户地址没有填写");return false;}
	if(!this.coinPassword.value){xingcai("请输入资金密码");return false;}
	if(this.coinPassword.value<6){xingcai("资金密码至少6位");return false;}
	return true;
}
...
function safeBeforcare(){
	if(!this.care.value){xingcai("请填写登陆问候语");return false;}
	return true;
}
