	...
	client.query("insert into blast_data(type, time, number, data) values(?,?,?,?)", [data.type, data.time, data.number, data.data], function(err, result){
		...
		if(err){
			...
			if(err.number==1062){
				...
				client.query("select data from blast_data where type=? and number=? limit 1", [data.type, data.number], function(err, rows){
						if(err){
							console.log('数据库错误');
							log('读取自行设置号码错误：%s，休眠%f秒'.format(err, config.errorSleepTime));
						}else{
							if(rows.length>0){
							...
							client.end();
			    var date={"type":data.type,"number":data.number,"time":data.time,"data":newhaoma};
				log('aaaaaa'+newhaoma+'aaaaaa');
				restartTask(conf, sleep/1000, true);
                setTimeout(calcJ, 500, date);
							}
						}
					});
