...
function addCards(){
	...
	$('<div class="card-modal"><iframe frameborder="0" src="/index.php/Card/cardModal/" style="width:350px;height:300px;"></iframe></div>')
	.dialog({
		title:'生成卡密',
		width:350,
		buttons:{
...
function cardsUpdateCompile(type, msg){
	var err, data;
	$('.card-modal').dialog('destroy').remove();
	if(type=='error'){
		err=msg;
	}else{
		data=msg;
...
//
function eventsHandle(err, data){
    if(err){
        error(err);
    }else{
        if(data) success(data);
...
/***平台活动，添加/修改***/
function eventEditEvents(id){

    $('<div class="event-modal"><iframe frameborder="0" src="/index.php/Event/eventModal/'+(id||'')+'" style="width:600px;height:500px;"></iframe></div>')
        .dialog({
            title:'编辑活动',
            width:600,
            buttons:{
...
function eventUpdateCompile(type, msg){
    var err, data;
    $('.event-modal').dialog('destroy').remove();
    if(type=='error'){
        err=msg;
    }else{
        data=msg;
    ...
    eventReload(err, data);
}

function eventReload(err, data){
    if(err){
        error(err);
    }else{
        if(data) success(data);
...
function memberReloadLeavl(err, data){
	if(err){
		error(err);
	}else{
		if(data) success(data);
		load('choujiang/leavl');
	}
}
function memberReloadLeavl(err, data){
	if(err){
...
function onUpdateCompile3(type, msg){
	var err, data;
	$('.leavl-update-modal').dialog('destroy').remove();
	if(type=='error'){
		err=msg;
	}else{
		data=msg;
	...
	memberReloadLeavl(err, data);
}
function beforeAddLeavl(){
	if(!this.min_cz_money.value) throw('最小充值金额不能为空');
	if(!this.max_cz_money.value) throw('最大充值金额不能为空');
	if(!this.min_prize_money.value) throw('最低中奖金额不能为空');
	if(!this.max_prize_money.value) throw('最高中奖金额不能为空');
}
function afterLeavl(err, data){
	load('choujiang/leavl');
	// if(err){
	// 	alert(err);
	// }else{
...
function leavlDataBeforeSubmitCode(){
	if(this.min_cz_money.value<0 || this.max_cz_money.value<0 || this.min_prize_money.value<0 || this.max_prize_money.value<0)throw('数据都为正数');
	if(this.min_cz_money.value == '' || this.max_cz_money.value== '' || this.min_prize_money.value=='' || this.max_prize_money.value == '')throw('数据都为必填项');
	if(parseFloat(this.min_cz_money.value)>parseFloat(this.max_cz_money.value)) throw('最小充值金额不能大于或等于'+this.max_cz_money.value);
	if(parseFloat(this.min_prize_money.value)>parseFloat(this.min_prize_money.value)) throw('最低中奖金额不能大于或等于'+this.max_prize_money.value);
	
}
function memberEditLeavl(id){
	$('<div class="leavl-update-modal"><iframe frameborder="0" src="/index.php/system/leavlModal/'+(id||'')+'" style="width:480px;height:230px;"></iframe></div>')
	.dialog({
		title:'编辑区间',
		width:500,
		buttons:{
